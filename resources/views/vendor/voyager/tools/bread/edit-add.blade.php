...
@section('javascript')
    ...
    <script>
        ...
        window.voyagerModelsNamespace = @json(config('voyager.models.namespace', 'App\\Models\\'));
        function tableToModelName(tableStr) {
            if (!tableStr) return '';
            var s = tableStr;
            if (s.endsWith('ies')) s = s.slice(0, -3) + 'y';
            else if (/\b(ses|xes|zes|ches|shes)$/.test(s)) s = s.slice(0, -2);
            else if (s.endsWith('s') && !s.endsWith('ss')) s = s.slice(0, -1);
            return s.split('_').map(function(p) { return p.charAt(0).toUpperCase() + (p.slice(1) || '').toLowerCase(); }).join('');
        }
        function setRelationshipModelFromTable($select) {
            var tableVal = $select.val();
            var modelName = tableToModelName(tableVal);
            if (!modelName) return;
            var ns = window.voyagerModelsNamespace || '';
            var fullName = (ns.replace(/\\+$/, '') ? ns.replace(/\\+$/, '') + '\\' : '') + modelName;
            var $block = $select.closest('.relationship_details');
            if (!$block.length) $block = $select.closest('#new_relationship_modal');
            var $input = $block.find('input[name="relationship_model"], input[name^="relationship_model_"]').first();
            if ($input.length) $input.val(fullName);
        }
        ...
        $(function () {
            ...
            $('.btn-new-relationship').click(function(){
                ...
                setRelationshipModelFromTable($('#new_relationship_modal select.relationship_table'));
            ...
            $('.relationship_table').on('change', function(){
                ...
                setRelationshipModelFromTable($(this));
            });
