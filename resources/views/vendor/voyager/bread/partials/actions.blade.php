@if($data)
    ...
    @can ($action->getPolicy(), $data)
        @if ($action->shouldActionDisplayOnRow($data))
            @if(method_exists($action, 'usePost') && $action->usePost())
                <form action="{{ $action->getRoute($dataType->name) }}" method="POST" class="form-inline" style="display:inline;">
                    @csrf
                    <button type="submit" title="{{ $action->getTitle() }}" {!! $action->convertAttributesToHtml() !!}>
